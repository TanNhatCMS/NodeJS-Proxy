...
function binaryToString(data){
    ...
    // Decode binary data to text if necessary
    if (Buffer.isBuffer(data)) {
        decodedText = data.toString('utf8'); // Assuming UTF-8 encoding
    } else if (typeof data === 'string') {
        decodedText = data;
    } else {
        throw new Error('Input is not a valid string or binary data');
    }
return decodedText;
}
class Cloudflare {
    ...
    request(uri, options) {
        ...
        return this._rp(params)
            .then(res => {
                let result = Cloudflare.parse(res);
                // Redirect
                if(result.redirect) {
    ...
    static parse(response = {}) {
        ...
        const value = binaryToString(body);
        result.error = this.findError(value);
        if(result.error) return result;

        result.captcha = this.findCaptcha(value);
        if(result.captcha) return result;

        result.challenge = this.findChallenge(value, uri.host);
        if(result.challenge) {
...
module.exports = {Cloudflare, helpers};
